...
public class RedisController {
    ...
    public String redisValue(String testValue){
        ...
        if(redisUtil.hasKey(testValue)){
            System.out.println(redisTemplate.getExpire(testValue));
            System.out.println("存在key，将从redis里获取");
        }else{
            System.out.println("不存在，查数据库，查出结果放入缓存，在返回");
            redisUtil.set(testValue,"test1",5);
            System.out.println(redisTemplate.getExpire(testValue));
        }
        return redisUtil.get(testValue).toString();
    }
    //哈希
    @ResponseBody
    ...
    public Map redisHashMap(String testValue){
        //添加单个
        redisUtil.hset("testhash","id","001");
        //添加多个
        HashMap map = new HashMap<>();
        map.put("name","小明");
        map.put("sex","男");
        redisUtil.hmset("testhash",map);
        //键集合
        ...
        //获取整个map
        return redisUtil.hmget("testMap");

    }
    //集合
    @ResponseBody
    @GetMapping("/redisSet")
    public Set<Object> setRedis(String set){
    ...
    public List<String> redisList(){
       //表头插入一个
        ...
        return redisTemplate.opsForList().range("left-list",0,-1);
    }
    //有序集合
    /*@ResponseBody
