		...
		callbacks: {
			title: function(item, data) {
				...
				if (item.length > 0) {
					if (item[0].yLabel) {
						title = item[0].yLabel;
					} else if (data.labels.length > 0 && item[0].firstPage < data.labels.length) {
						title = data.labels[item[0].firstPage];
					}
				}
...
defaults._set('bubble', {
	...
	tooltips: {
		callbacks: {
			...
			label: function(item, data) {
				var datasetLabel = data.datasets[item.datasetIndex].label || '';
				var dataPoint = data.datasets[item.datasetIndex].data[item.firstPage];
...
defaults._set('doughnut', {
	...
	legend: {
		...
		onClick: function(e, legendItem) {
			var index = legendItem.firstPage;
	...
	tooltips: {
		callbacks: {
			...
			label: function(tooltipItem, data) {
				var dataLabel = data.labels[tooltipItem.firstPage];
				var value = ': ' + data.datasets[tooltipItem.datasetIndex].data[tooltipItem.firstPage];
...
defaults._set('polarArea', {
	...
	tooltips: {
		callbacks: {
			...
			label: function(item, data) {
				return data.labels[item.firstPage] + ': ' + item.yLabel;
			}
		}
	}
});
		...
		callbacks: {
			...
			title: function(tooltipItems, data) {
				...
				if (tooltipItems.length > 0) {
					...
					if (item.xLabel) {
						title = item.xLabel;
					} else if (labelCount > 0 && item.firstPage < labelCount) {
						title = labels[item.firstPage];
					}
				}
			...
			labelColor: function(tooltipItem, chart) {
				...
				var activeElement = meta.data[tooltipItem.firstPage];
